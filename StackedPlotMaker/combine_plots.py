...
for plotname, plotset in plotdict.items():
    ...
    infiles = []
    for key in plotset:
        matching_files = [f for f in os.listdir(args.indir) if f == f"{key}.png"]
        infiles.extend(matching_files)

    #infiles = [f for f in os.listdir(args.indir) if f.endswith('.png')]
    ...
    images = [Image.open(os.path.join(args.indir, f)) for f in infiles]
    w, h = images[0].size
    n = len(images)
    ...
    outname = os.path.join(args.indir, f"combined_{plotname}.png")
