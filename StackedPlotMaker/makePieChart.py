...
def main():
    ...
    for campaign, name in campaigndict.items():
        for ch in channels:
            ...
            plot_background_pie(csvfile, name, ch)
...
def plot_background_pie(csv_file, campaign, channel):
    jobname = os.path.basename(os.path.dirname(csv_file))
    outdir = f"pie/{jobname}"
